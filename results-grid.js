  ...
  function getContentTypeLabels(keywords) {
    ...
    if (!hasRealMetadata) {
      ...
      let sourceMetadata = {
        ...
        viral_score: results?.viral_score,
      ...
      // If no top-level metadata, get from first clip
      if (!sourceMetadata.analysis_mode && results?.clips && results.clips.length > 0) {
        const firstClipAnalysis = results.clips[0]?.analysis;
        sourceMetadata = {
          analysis_mode: firstClipAnalysis?.analysis_mode,
          keywords: firstClipAnalysis?.keywords,
          viral_score: firstClipAnalysis?.viral_score,
          total_moments: firstClipAnalysis?.total_moments,
          video_duration: firstClipAnalysis?.video_duration,
          content_type: firstClipAnalysis?.content_type,
      ...
      detailsPanel.innerHTML = `
        <div class="metadata-details-content">
          ...
          <div class="metadata-row">
            ...
            <span class="metadata-value">${metadata.keywords.length > 0 ? getContentTypeLabels(metadata.keywords) : '<span style="opacity: 0.6">No keywords detected</span>'}</span>
          </div>
