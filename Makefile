...
# Configuration - AgentOS Port Range: 8100-8199, Redis: 6380
API_PORT = 8101
FRONTEND_PORT = 8100
ADMIN_CLEAN_PORT = 8104
REDIS_PORT = 6380
WEBSOCKET_PORT = 8165
FLOWER_PORT = 8155
PID_DIR = .pids
...
api-start: ## Start API server (port 8001)
	...
	@if [ -f $(PID_DIR)/api.pid ] && kill -0 `cat $(PID_DIR)/api.pid` 2>/dev/null; then \
		...
		nohup ./venv/bin/python -m uvicorn api.main:app --host 0.0.0.0 --port $(API_PORT) > logs/api.log 2>&1 & echo $$! > $(PID_DIR)/api.pid; \
...
frontend-start: ## Start frontend server (port 8000)
	...
	@if [ -f $(PID_DIR)/frontend.pid ] && kill -0 `cat $(PID_DIR)/frontend.pid` 2>/dev/null; then \
		...
		nohup ./venv/bin/python -m http.server $(FRONTEND_PORT) > logs/frontend.log 2>&1 & echo $$! > $(PID_DIR)/frontend.pid; \
...
admin-clean-start: ## Start clean admin dashboard server (port 8004)
	...
	@if [ -f $(PID_DIR)/admin-clean.pid ] && kill -0 `cat $(PID_DIR)/admin-clean.pid` 2>/dev/null; then \
		...
		nohup ./venv/bin/python -m http.server $(ADMIN_CLEAN_PORT) --directory ui-admin-clean > logs/admin-clean.log 2>&1 & echo $$! > $(PID_DIR)/admin-clean.pid; \
...
celery-worker-start: ## Start Celery worker (industry standard)
	...
	@if [ -f $(PID_DIR)/celery-worker.pid ] && kill -0 `cat $(PID_DIR)/celery-worker.pid` 2>/dev/null; then \
		...
		nohup ./venv/bin/python -m celery -A core.celery_app:celery_app worker --loglevel=info --concurrency=4 --logfile=logs/celery/worker.log > logs/celery/worker-output.log 2>&1 & echo $$! > $(PID_DIR)/celery-worker.pid; \
...
celery-worker-start-multi: ## Start multiple Celery workers (high concurrency)
	...
	@for i in 1 2 3 4 5; do \
		if [ -f $(PID_DIR)/celery-worker-$$i.pid ] && kill -0 `cat $(PID_DIR)/celery-worker-$$i.pid` 2>/dev/null; then \
			...
			nohup ./venv/bin/python -m celery -A core.celery_app:celery_app worker --loglevel=info --concurrency=4 --hostname=worker$$i@%h --logfile=logs/celery/worker-$$i.log > logs/celery/worker-$$i-output.log 2>&1 & echo $$! > $(PID_DIR)/celery-worker-$$i.pid; \
...
celery-beat-start: ## Start Celery Beat scheduler (for periodic tasks)
	...
	@if [ -f $(PID_DIR)/celery-beat.pid ] && kill -0 `cat $(PID_DIR)/celery-beat.pid` 2>/dev/null; then \
		...
		nohup ./venv/bin/python -m celery -A core.celery_app:celery_app beat --loglevel=info --logfile=logs/celery/beat.log > logs/celery/beat-output.log 2>&1 & echo $$! > $(PID_DIR)/celery-beat.pid; \
...
celery-flower-start: ## Start Celery Flower monitoring (Web UI)
	...
	@if [ -f $(PID_DIR)/celery-flower.pid ] && kill -0 `cat $(PID_DIR)/celery-flower.pid` 2>/dev/null; then \
		...
		nohup ./venv/bin/python -m celery -A core.celery_app:celery_app flower --port=5555 --logfile=logs/celery/flower.log > logs/celery/flower-output.log 2>&1 & echo $$! > $(PID_DIR)/celery-flower.pid; \
...
celery-status: ## Show Celery system status
	...
	@./venv/bin/python -m celery -A core.celery_app:celery_app inspect active 2>/dev/null || echo "  $(RED)No workers responding$(NC)"
	@echo "$(YELLOW)Queues:$(NC)"
	@./venv/bin/python -m celery -A core.celery_app:celery_app inspect active_queues 2>/dev/null || echo "  $(RED)No queue info available$(NC)"
	@echo "$(YELLOW)Registered tasks:$(NC)"
	@./venv/bin/python -m celery -A core.celery_app:celery_app inspect registered 2>/dev/null | head -20 || echo "  $(RED)No registered tasks$(NC)"

celery-purge: ## Purge all tasks from queues (⚠️ DESTRUCTIVE)
	@echo "$(RED)⚠️  WARNING: This will delete all queued tasks!$(NC)"
	@read -p "Are you sure? (y/N): " confirm && [ "$$confirm" = "y" ] || exit 1
	@./venv/bin/python -m celery -A core.celery_app:celery_app purge -f
...
websocket-start: ## Start WebSocket server (port 8765)
	...
	@if [ -f $(PID_DIR)/websocket.pid ] && kill -0 `cat $(PID_DIR)/websocket.pid` 2>/dev/null; then \
		...
		nohup ./venv/bin/python websockets/websocket_server.py > logs/websocket.log 2>&1 & echo $$! > $(PID_DIR)/websocket.pid; \
...
clean: ## Clean PID files and stop orphaned processes
	...
	@echo "$(YELLOW)Killing Celery workers (including stuck ones)...$(NC)"
...
cleanup-all: ## Clean up ALL AgentOS processes (including orphaned development workers)
	...
	@echo "  - Celery workers (force kill)..."
	@pkill -9 -f "celery.*worker" 2>/dev/null || true
	@pkill -9 -f "celery.*beat" 2>/dev/null || true
	@pkill -9 -f "celery.*flower" 2>/dev/null || true
	@echo ""
	@echo "$(YELLOW)Checking for remaining AgentOS processes...$(NC)"
	@ps aux | grep -E "(video_worker|api\.main|websocket_server|http\.server.*800|celery)" | grep -v grep || echo "  $(GREEN)No remaining processes found$(NC)"
	@echo ""
