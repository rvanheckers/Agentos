...
class AnalyticsService:
    ...
    def get_analytics(self, timeframe: str = "7d", is_admin: bool = False) -> Dict[str, Any]:
        ...
        try:
            if self.db_service:
                ...
                # ---- Calculate timeframe window (and actually use it) ----
                ...
                def _parse_dt(val) -> datetime | None:
                    # Accept datetime obj, iso string, or None
                    if val is None:
                        return None
                    ...
                    try:
                        ...
                        t = val.replace("Z", "+00:00")
                        return datetime.fromisoformat(t)
                    except Exception:
                        return None

                filtered_jobs = [j for j in all_jobs if (_dt := _parse_dt(j.get("created_at"))) and _dt >= start_dt]
                ...
                # Optional aggregates (als je Jobs opslaat met deze velden)
                def _sum_safe(items, key: str) -> int:
                    s = 0
                    for it in items:
                        try:
                            v = it.get(key)
                            ...
                            pass
                    return s
                moments_detected = _sum_safe(filtered_jobs, "total_moments")
                clips_produced = _sum_safe(filtered_jobs, "clips_produced")
    ...
    def get_system_config(self, is_admin: bool = False) -> Dict[str, Any]:
        ...
        try:
            # NOTE: waarden hier kunnen het beste uit env komen zodat het matcht met je Celery/DB setup
            import os
            config = {
                "system": {
                    "version": "1.0.0",
                ...
                "storage": {
                    "input_directory": os.getenv("INPUT_DIR", "./io/input"),
                    "output_directory": os.getenv("OUTPUT_DIR", "./io/output"),
                    "temp_directory": os.getenv("TEMP_DIR", "./io/temp"),
                    "max_file_size": "500MB"
                },
                "database": {
                    "type": os.getenv("DB_TYPE", "postgres"),
                    "url": os.getenv("DATABASE_URL", "postgresql+psycopg://user:pass@localhost:5432/agentos"),
    ...
    def _get_active_workers_count(self) -> int:
        ...
        try:
            # 1) Probeer Celery inspect (cross-platform)
            try:
                from core.celery_app import celery_app
                insp = celery_app.control.inspect(timeout=1.0)
                active = insp.active() or {}
                worker_count = len(active.keys())
                logger.info(f"Found {worker_count} active Celery workers via inspect()")
                return worker_count
            except Exception:
                # 2) Fallback: ps-heuristiek (POSIX)
                import subprocess
                result = subprocess.run(["ps", "aux"], capture_output=True, text=True)
                worker_lines = [ln for ln in result.stdout.splitlines()
                                if "celery" in ln and "worker" in ln and "grep" not in ln]
                worker_count = len(worker_lines)
                logger.info(f"Found {worker_count} active Celery workers (ps fallback)")
                return worker_count
