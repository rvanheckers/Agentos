...
class RateLimiter:
    ...
    async def _check_token_bucket(self, key: str, rate_limit: RateLimit) -> bool:
        ...
        if state.tokens >= 1.0:
            ...
            logger.debug(
                f"Rate limit check passed (token bucket)",
                extra={
                    "key": key,
            ...
            logger.info(
                f"Rate limit exceeded (token bucket)",
                extra={
                    "key": key,
                    "tokens_remaining": state.tokens,
                    "required": 1.0
    ...
    async def _check_sliding_window(self, key: str, rate_limit: RateLimit) -> bool:
        ...
        if current_count < rate_limit.requests:
            logger.debug(
                f"Rate limit check passed (sliding window)",
                extra={
                    "key": key,
            ...
            logger.info(
                f"Rate limit exceeded (sliding window)",
                extra={
                    "key": key,
                    "current_count": current_count,
                    "limit": rate_limit.requests
    ...
    async def _check_fixed_window(self, key: str, rate_limit: RateLimit) -> bool:
        ...
        if current_count < rate_limit.requests:
            ...
            logger.debug(
                f"Rate limit check passed (fixed window)",
                extra={
                    "key": key,
            ...
            logger.info(
                f"Rate limit exceeded (fixed window)",
                extra={
                    "key": key,
                    "current_count": current_count,
                    "limit": rate_limit.requests
