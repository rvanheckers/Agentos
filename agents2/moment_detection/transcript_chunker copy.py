...
Process the chunk below and detect moments:"""
    ...
    def _analyze_single_chunk(
        ...
        try:
            ...
            # Call Claude API with cache headers
            ...
            # Extract moments and adjust timestamps
    ...
    def _calculate_time_offset(self, chunk: TranscriptChunk, constraints: Dict[str, Any]) -> float:
        """
        Calculate time offset for chunk using words_per_minute

        CORRECTED IMPLEMENTATION:
        - Uses words_per_minute from constraints
        - Assumes ~5 chars per word (industry average)
        - Maps character position to timestamp

        Args:
            chunk: TranscriptChunk with start_offset
            constraints: Must include 'words_per_minute'
        ...
        Example:
            ...
            wpm = 160 (typical podcast speed)
            words = 10000 / 5 = 2000 words
            minutes = 2000 / 160 = 12.5 minutes
            seconds = 12.5 * 60 = 750 seconds
        """
        wpm = float(constraints.get('words_per_minute', 160))  # Default podcast speed

        # Calculate seconds per character
        # WPM = words/minute, assume 5 chars/word
        chars_per_minute = wpm * 5
        sec_per_char = 60.0 / chars_per_minute

        # Calculate offset
        time_offset_s = chunk.start_offset * sec_per_char
    ...
    def deduplicate_moments(
        ...
        for moment in sorted_moments:
            ...
            for i, existing in enumerate(deduplicated):
                ...
                if time_overlap:
                    # Check content similarity (Jaccard on key_phrase words)
                    jaccard = self._calculate_jaccard_similarity(
                        moment.get('key_phrase', ''),
                        existing.get('key_phrase', '')
                    )
    ...
    def calculate_cost(self) -> Dict[str, Any]:
        ...
        total_input = self.token_counts['input'] + self.token_counts['cache_read']
        cache_hit_rate = self.token_counts['cache_read'] / max(1, total_input)

        return {
            'total_cost': round(cost, 4),
            'token_counts': dict(self.token_counts),
            'cache_hit_rate': round(cache_hit_rate, 3),
            'cost_per_chunk': round(cost / max(1, len(self.token_counts)), 4)
        }
        ...
        with ThreadPoolExecutor(max_workers=self.batch_size) as executor:
            ...
            for batch_idx, batch in enumerate(batches):
                ...
                if batch_idx < len(batches) - 1:  # Don't wait after last batch
                    logger.info(f"⏳ Rate limit: waiting {self.batch_delay_s:.1f}s before next batch")
