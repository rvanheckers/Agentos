...
export class VideoStore {
  constructor() {
    this.state = {
      ...
      uploadSource: null,
      uploadError: null,
      jobId: null, // Backend job ID for tracking
  ...
  setCurrentVideo(file, metadata = null) {
    // file can be a File object or a videoInput object with url/jobId
    const isFileObject = file instanceof File;
    const jobId = isFileObject ? null : (file?.jobId || null);

    this.setState({
      currentVideo: file,
      videoMetadata: metadata,
      uploadMethod: isFileObject ? 'file' : 'url',
      uploadSource: isFileObject ? file.name : (file?.url || file),
      uploadError: null,
      jobId: jobId
    });

    if (jobId) {
      console.log('📋 VideoStore: Job ID set:', jobId);
    }
  }
  ...
  setProcessingResults(results) {
    this.setState({
      ...
      processingResults: {
        ...results,
        // Ensure top-level metadata is preserved for frontend compatibility
