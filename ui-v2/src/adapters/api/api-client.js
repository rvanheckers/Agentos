...
export class APIClient {
  constructor() {
    this.baseUrl = 'http://localhost:8101'; // AgentOS port range: 8100-8199
  ...
  async processVideoUrl(url, intent, options = {}) {
    ...
    try {
      ...
      // Return data with job_id at top level for compatibility
      return {
        ...result,
        job_id: result.data?.job_id || result.data?.id,
  ...
  setupWebSocket(sessionId) {
    ...
    try {
      this.websocket = new WebSocket(`ws://localhost:8165/ws?session_id=${sessionId}`);
   ...
   */
  async getCompleteMetadata(jobId) {
    const response = await fetch(`${this.baseUrl}/api/jobs/${jobId}/complete-metadata`);
    if (!response.ok) {
        if (response.status === 410) {
            throw new Error('Content has expired and been deleted for privacy');
        }
   ...
   */
  async deleteMyData(sessionId) {
    const response = await fetch(`${this.baseUrl}/api/privacy/delete-my-data`, {
        method: 'POST',
