...
import { clipSettingsManager } from '../../clip-settings/clip-settings-manager.js';
...
export class UploadZone {
  ...
  async handleFileUpload(file) {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Get clip settings
        const userPreferences = clipSettingsManager.getUserPreferences();
        console.log('📏 Using clip settings:', userPreferences);
        ...
        const videoInput = {
          ...
          source: 'file_upload',
          user_preferences: userPreferences  // Add clip settings
        };
  ...
  async handleUrlUpload() {
    ...
    try {
      ...
      console.log('🔍 Calling uploadService.processUrl...');
      console.log('🔍 uploadService exists:', !!this.uploadService);
      const result = await this.uploadService.processUrl(url, (progress) => {
        console.log('🔍 URL progress:', progress);
        this.stores.video.setUploadProgress(progress);
      });
      console.log('🔍 uploadService.processUrl returned:', result);
      ...
      if (result.success) {
        ...
        // Get clip settings
        const userPreferences = clipSettingsManager.getUserPreferences();
        console.log('📏 Using clip settings for URL:', userPreferences);

        // Start video processing with original URL - backend handles download
        console.log('🎬 Job created with ID:', result.jobId);
        this.stores.startVideoUpload({
          url: url,  // Original YouTube URL
          source: 'url',
          originalUrl: url,
          jobId: result.jobId,  // Pass job ID from backend
          user_preferences: userPreferences  // Add clip settings
        }, 'url');
        ...
        // Navigate to processing step after short delay (same as file upload)
        console.log('🚀 Navigating to processing step...');
        setTimeout(() => {
          console.log('🚀 Calling navigateToStep("processing")');
          this.stores.ui.navigateToStep('processing');
        }, 1500);
  ...
  resetUploadZone() {
    ...
    this.removeEventListeners(); // ALTIJD eerst oude listeners verwijderen!
