...
export class UploadService {
  ...
  async processUrl(url, onProgress = null) {
    ...
    console.log('🔍 this.apiClient exists:', !!this.apiClient);
    console.log('🔍 this.apiClient.processVideoUrl exists:', !!this.apiClient?.processVideoUrl);
    ...
    try {
      ...
      if (this.apiClient.processVideoUrl) {
        ...
        console.log('🔍 About to call processVideoUrl with:', url);
        ...
        console.log('🔍 Calling processVideoUrl now...');
        const result = await this.apiClient.processVideoUrl(url, 'url_processing');
        console.log('🔍 processVideoUrl returned:', result);

        if (onProgress) onProgress(100); // Complete

        const returnValue = {
          success: true,
          videoPath: url, // Keep original URL for backend processing
          metadata: result.metadata || { source_url: url },
          message: result.message || 'URL processed successfully',
          jobId: result.job_id // Pass through job ID from backend
        };
        console.log('🔍 Returning from processUrl:', returnValue);
        return returnValue;
      }
