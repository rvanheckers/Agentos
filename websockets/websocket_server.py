...
# Deprecated import removed - using new websockets API
import signal
import sys
...
class AdminWebSocketServer:
    ...
    def __init__(self, redis_host: str = None, redis_port: int = None):
        ...
        redis_port = redis_port or int(os.environ.get("REDIS_PORT", "6380"))
        self.redis_client = redis.Redis(host=redis_host, port=redis_port, decode_responses=True)
        self.pubsub = self.redis_client.pubsub()
        # Using shared database pool

        # V4 ROOM-BASED CONNECTION MANAGEMENT
        self.rooms: Dict[str, Set[websockets.WebSocketServerProtocol]] = {  # room_name -> websockets
        ...
        self.connections: Dict[str, Set[websockets.WebSocketServerProtocol]] = {}  # job_id -> websockets
        self.all_connections: Set[websockets.WebSocketServerProtocol] = set()      # All connections
        self.client_subscriptions: Dict[websockets.WebSocketServerProtocol, Set[str]] = {}  # client -> job_ids
        self.admin_connections: Set[websockets.WebSocketServerProtocol] = set()    # admin clients
    ...
    async def subscribe_to_job(self, websocket: websockets.WebSocketServerProtocol, job_id: str):
    ...
    async def unsubscribe_from_job(self, websocket: websockets.WebSocketServerProtocol, job_id: str):
    ...
    async def subscribe_to_admin_updates(self, websocket: websockets.WebSocketServerProtocol, client_id: str = None):
    ...
    async def unsubscribe_from_admin_updates(self, websocket: websockets.WebSocketServerProtocol):
    ...
    async def subscribe_to_room(self, websocket: websockets.WebSocketServerProtocol, room: str):
    ...
    async def unsubscribe_from_room(self, websocket: websockets.WebSocketServerProtocol, room: str):
    ...
    async def _cleanup_disconnected_client(self, websocket: websockets.WebSocketServerProtocol, room: str = None):
    ...
    async def send_job_status(self, websocket: websockets.WebSocketServerProtocol, job_id: str):
    ...
    async def send_queue_stats(self, websocket: websockets.WebSocketServerProtocol):
...
async def start_websocket_server(host: str = "localhost", port: int = None):
    """Start V4 WebSocket server with event integration"""
    import os
    if port is None:
        port = int(os.environ.get("WEBSOCKET_PORT", "8165"))
    ...
    server = await websockets.serve(
        ...
        extensions=None,
        subprotocols=None,
        # extra_headers parameter removed in websockets v12+
