...
export class ProcessingPanel {
  ...
  startProcessingTimer() {
    ...
    this.processingTimer = setInterval(() => {
      ...
      if (timerMessage) {
        ...
        if (elapsed < 30) {
          message = i18n.translate('processing.timer.starting') || 'Starting video analysis...';
        } else if (elapsed < 60) {
          message = i18n.translate('processing.timer.downloading') || 'Downloading and preparing video...';
        } else if (elapsed < 120) {
          message = i18n.translate('processing.timer.analyzing') || 'AI is analyzing your content...';
        } else if (elapsed < 180) {
          message = i18n.translate('processing.timer.patience') || 'This can take 3-5 minutes for longer videos...';
        } else {
          message = i18n.translate('processing.timer.almost') || 'Almost done, finalizing clips...';
        }
  ...
  stopProcessingTimer() {
    if (this.processingTimer) {
      ...
      if (this.processingStartTime) {
        ...
        const timerMessage = document.getElementById('timerMessage');
        if (timerMessage) {
          const completedText = i18n.translate('processing.timer.completed') || 'Processing completed in';
          timerMessage.textContent = `✅ ${completedText} ${minutes}m ${seconds}s`;
        }
      }
    }
  }
  ...
  async handleAsyncComplete(result) {
    ...
    this.stores.video.setProcessingResults({
      ...
      // CRITICAL FIX: Include YouTube metadata from API response
      video_title: clipsData?.video_title,
      content_type: clipsData?.content_type,
      youtube_metadata: clipsData?.youtube_metadata || {},
      analysis_mode: clipsData?.analysis_mode,
      keywords: clipsData?.keywords || [],
      viral_score: clipsData?.viral_score,
      total_moments: clipsData?.total_moments,
      video_duration: clipsData?.video_duration
    });
